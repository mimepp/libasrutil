...
int main(int argc, char *argv[])
{
	...
	char *to_data = NULL;
	int to_len = 0;
	asrutil_text_translate_mode mode = ASRUTIL_MODE_EN_2_ZH;
	...
	while (p_len < len) {
		...
		asrutil_voice_engine_convert(p, src_len, &en_data, &en_len);
		ASRUTIL_LOG("%s:%d, en_data: %s, en_len: %d\n", __FUNCTION__, __LINE__, en_data, en_len);

		asrutil_text_translate_convert(en_data, en_len, &to_data, &to_len, mode);
		ASRUTIL_LOG("%s:%d, to_data: %s, to_len: %d\n", __FUNCTION__, __LINE__, to_data, to_len);
		ASRUTIL_SAFE_FREE(to_data);

		ASRUTIL_SAFE_FREE(en_data);
		p += block_size;
		p_len += block_size;
	}
